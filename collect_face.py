...
def pre_resize(before, after, height=IMAGE_HEIGHT, filename="", antialias_enable=True):
    ...
    resize_img.save(after, 'png', quality=100)
...
if __name__ == "__main__":
    ...
    resize_files = glob.glob(resize_dir + "/*.png")
    ...
    for file_name in resize_files:
        ...
        for r in range(-12, 13, 4):
            ...
            for facerect in facerect_list:
                ...
                # 出力
                cv2.imwrite(trimming_dir + "/" + str(face_cnt) + ".png", resize(croped))
                face_cnt += 1

                # 反転画像も出力
                fliped = np.fliplr(croped)
                cv2.imwrite(trimming_dir + "/" + str(face_cnt) + ".png", resize(fliped))
                face_cnt += 1
